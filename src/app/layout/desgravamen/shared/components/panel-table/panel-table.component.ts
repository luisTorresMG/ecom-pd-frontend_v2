...
import { ModuleConfig } from '../../../../../layout/broker/components/module.config';
...
export class PanelTableComponent implements OnInit, OnChanges {
  ...
  @Input() valorBase: string = "";

  @Input() datosBase: any[] = [];
  @Output() datosBaseChange: EventEmitter<any> = new EventEmitter();
  ...
  ngOnChanges(changes) {
    ...
    }else if (changes.reload && changes.reload.currentValue == false && this.datosBase){
      if (this.datosBase.length){
      setTimeout(() => { this.reloadChange.emit(false); });
      this.initPagination();

      this.loaderChange.emit(true);

      this.pagination = Object.assign(this.pagination, this.pagination || {});
