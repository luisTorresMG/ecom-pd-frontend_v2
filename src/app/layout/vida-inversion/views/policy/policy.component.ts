...
export class PolicyComponent implements OnInit {
    ...
    historialEvaluaciones: any[] = [];    // DVP VIGP 10-03-2025
            ...
            if (res.P_COD_ERROR == 1) {
                ...
                this.list_benefeciarys = res.List_beneficiary.map(element => {
                    return {
                        ...
                        block_nationality: this.CONSTANTS.BLOCKED_NATIONALITIES.includes(parseInt(element.nnationality)) ? 1 : 0,
                    };
                })
    ...
    async cargarDatosInsured(res: any, search_type: any) {
        ...
        this.data_insured.nationality = { NNATIONALITY: parseInt(contracting_data.P_NNATIONALITY) };
                            ...
                            if (res.P_NCODE === "0") {
                                if (res.EListClient[0].P_SCLIENT == null) {
                                    ...
                                    if (res.EListClient.length === 1) {
                                        ...
                                        await this.getIdecon(2); // cambio TEMP
    ...
    ReturnQuotation() {
        if (this.comment == "") {
            ...
            const updateState = async () => {
                let stateItem = {
                    P_NID_COTIZACION: this.quotation_id,
                    P_SSTATREGT_COT: 29, // Estado OBSERVADA
                    P_NUSERCODE: this.cur_usr.id,
                    P_DDERIV_FIRMAS: null
                };

                try {
                    const response_upd = await this.quotationService.InsUpdCotiStatesVIGP(stateItem).toPromise();

                    if (response_upd.P_NCODE == 1) {
                        Swal.fire('Información', response_upd.P_MESSAGE, 'error');
                        return false;
                    }
                    return true;
                } catch (error) {
                    Swal.fire('Información', 'Error al actualizar el estado', 'error');
                    return false;
                }
            };

            updateState().then(success => {
                if (success) {
                    // Derivar a soporte comercial
                    const changeStatus = {
                        P_NID_COTIZACION: this.quotation_id,
                        P_NID_PROC: this.quotation.nid_proc,
                        P_NID_STATE_DER: 3, // soporte comercial
                        P_SCOMMENT: this.comment,
                        P_NUSERCODE: this.cur_usr.id,
                        P_NFLAG_COTIZACION: 1,
                        P_FLAG_FILE_CLIENTE: "0"
                    };
                    ...
                    this.quotationService.InsCommentsCotiVIGP(changeStatus).toPromise().then((res) => {
                        if (res.P_NCODE == 1) {
                            ...
                            Swal.fire('Información', 'La cotización fue observada y derivada a soporte comercial.', 'success');
    ...
    async EmitPolicy() {
        ...
        const policy = [{
            ...
            P_NPRODUCT: this.CONSTANTS.COD_PRODUCTO.toString(),
            P_DSTARTDATE: this.dstartdate,
            P_DEXPIRDAT: this.dexpirdat,
            P_SCOLTIMRE: 6,
            P_NPAYFREQ: 6,
            ...
            P_NUSERCODE: this.cur_usr.id,
            P_SCOMMENT: '',
            P_NAMO_AFEC: this.parse_amount, // Prima Neta (sin igv)
            P_NIVA: 0,  // Impuestos (igv)
            P_NAMOUNT: this.parse_amount, // Prima Bruta (con igv)
            P_NDE: 0,
            P_DSTARTDATE_ASE: this.dstartdate_ase,
            P_DEXPIRDAT_ASE: this.dexpirdat_ase
        }]

        let params = {
            Nid_cotizacion: this.quotation_id,
            Nid_proc: this.quotation.nid_proc,
            Ntype_transac: /*1*/ this.CONSTANTS.TRANSACTION_CODE.EMISION,
            Nusercode: this.cur_usr.id,
            Nbranch: this.CONSTANTS.RAMO,
            Nproduct: this.CONSTANTS.COD_PRODUCTO.toString(),
            Sjson_prt: JSON.stringify(policy)
        }

        myFormData.append('dataTransac', JSON.stringify(params));

        this.policyemit.insPreTransac(myFormData).toPromise().then((res) => {

            if (res.P_COD_ERR == 1) {
                Swal.fire('Información', res.P_MESSAGE, 'error');
            }
            else {
                Swal.fire('Información', 'La emisión se realizará el siguiente día hábil.', 'success').then(result => {
                    this.router.navigate(['extranet/vida-inversion/consulta-cotizacion-definitiva']);
                });
    ...
    // FIN VIGP DVP 14-03-2025

    validateEmitTransaction() {
        ...
        if (this.check_input_value == 0) {
            ...
            if (validate_error_insured.cod_error == 1) {
                ...
                Swal.fire('Información', validate_error_insured.message_error, 'error');
                return;
            }
        }

        let beneficiaries_with_blocked_nationality = 0;

        for (let index = 0; index < this.list_benefeciarys.length; index++) {
            this.list_benefeciarys[index].block_nationality == 1 ? beneficiaries_with_blocked_nationality++ : null;
        }

        if (beneficiaries_with_blocked_nationality > 0) {
            Swal.fire('Información', 'El o los beneficiario(s) son de nacionalidad estadounidense.', 'error');
            return;
        }
