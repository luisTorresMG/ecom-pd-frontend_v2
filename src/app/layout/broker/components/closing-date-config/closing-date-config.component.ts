/*************************************************************************************************/
/*  NOMBRE              :   CLOSING-DATE-CONFIG.COMPONENT.HTML                                   */
...
import { ClosingDateConfigService } from '../../../backoffice/services/closing-date-config/closing-date-config.service';
...
@Component({
    selector: 'app-closing-date-config',
    templateUrl: './closing-date-config.component.html',
    styleUrls: ['./closing-date-config.component.css'],
    providers: [NgbModalConfig, NgbModal]
})

export class ClosingDateConfigComponent implements OnInit {
    ...
    constructor(
        ...
        private ClosingDateConfigService: ClosingDateConfigService,
    ...
    getParams = () => {
        ...
        return forkJoin([$estados, $origen, $meses]).subscribe(
    ...
    mostrarModalAgregar(content: any) {
        ...
        this.filtroRamos = {};
        this.filtroRamos.NNUMORI = this.filtroConfig.NNUMORI;
    ...
    mostrarModalModificar(content: any, item) {
        ...
        this.valueFinEditRange = new Date(this.valueFinEditRange.getFullYear(), monthFormat + 1, 0);
        this.filtroRamos = {};
        this.filtroRamos.NNUMORI = this.grillaConfig.NNUMORI;
    ...
    listarRamos = (filtro) => {
        this.ClosingDateConfigService.listarRamos(filtro).subscribe(
            res => {
