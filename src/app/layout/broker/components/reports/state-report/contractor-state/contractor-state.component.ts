...
//Servicios
import { StateReportService } from '../../../../services/report/state-report.service';
...
export class ContractorStateComponent implements OnInit {
  ...
  ngOnInit() {
    if (
      AccessFilter.hasPermission(ModuleConfig.ViewIdList['account_state']) ==
      false
    )
    ...
    if (
      ...
      this.contractor.Id.toString().trim() == ''
    )
      this.router.navigate(['/extranet/home']);
    ...
    if (
      ...
      if (!this.mainFormGroup.controls.endDate.valid) {
        if (this.mainFormGroup.controls.endDate.hasError('required'))
          errorList.push('La fecha final es requerida.');
        else errorList.push(this.invalidEndDateMessage);
      }

      if (this.contractor.Id == null || this.contractor.Id.trim() == '')
        errorList.push('El Id del cliente no es válido.');
  ...
  enable() {
    Swal.fire({
      ...
      text:
        'El cliente posee ' +
        this.contractor.LatePaymentDays +
        ' días de morosidad ¿Desea habilitar sus movimientos?',
      icon: 'warning',
  ...
  seeQualificationTracking() {
    const modalRef = this.modalService.open(
      CreditQualificationRecordComponent,
      {
        size: 'lg',
    ...
    modalRef.componentInstance.qualificationTypeList =
      this.qualificationTypeList;

    modalRef.result.then(
      (contractor) => {
        //Esperamos los datos del broker de respuesta
        if (contractor != null) {
          this.contractor.LastCreditEvaluationId =
            contractor.LastCreditEvaluationId;
          this.contractor.LastCreditEvaluationName =
            contractor.LastCreditEvaluationName;
        }
      },
      (reason) => {}
    );
  ...
  goBack() {
    sessionStorage.removeItem('cs-contractor');
    this.router.navigate(['/extranet/state-report']);
  }
}
