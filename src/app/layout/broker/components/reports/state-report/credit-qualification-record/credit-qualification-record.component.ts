...
export class CreditQualificationRecordComponent implements OnInit {
            ...
            (res) => {
              if (res.StatusCode == 0) {
                ...
                this.contractor.LastCreditEvaluationName =
                  this.qualificationTypeList[value];
                Swal.fire({
                  title: 'Información',
  ...
  firstSearch(isFirstSearch: boolean) {
    ...
    if (
      ...
      this.contractor.Id.toString().trim() != null
    ) {
      ...
      this.filter.Qualification =
        this.mainFormGroup.controls.qualification.value;
      ...
      if (
        ...
        if (this.mainFormGroup.controls.endDate.valid == false) {
          if (this.mainFormGroup.controls.endDate.hasError('required'))
            errorList.push('La fecha de fin es requerida.');
