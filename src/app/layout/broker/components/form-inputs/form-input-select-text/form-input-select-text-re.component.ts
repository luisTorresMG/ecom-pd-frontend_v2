...
export class FormInputSelectTextREComponent implements OnInit,OnChanges,OnDestroy {
  ...
  placeholderValue: string = "Seleccionar"; 
  ...
  ngOnInit() {
    this.opcionesFiltradas = this.items;
    if (!this.value) {
      this.onClear();
    }
  ...
  ngOnDestroy() {
    ...
    toggleInput(): void {
        ...
        // Limpiar filtro cuando se muestra el input
      ...
      filtrarOpciones(): void {
        this.opcionesFiltradas = this.items.filter(opcion =>
          opcion.text.toLowerCase().includes(this.filtro.toLowerCase())
        );
      }
