...
export class TransactEvaluationComponent implements OnInit {
  ...
  async getInfoQuotation(){
    ...
    async getInfoComer() {
        await this.policyService.getPolicyEmitComer(this.quotationNumber, 0, this.sAbrTran).toPromise().then(
            res => {
  ...
  trxDirect(objeto){
    ...
    async OpenModalPagos(paramsTrx) {
      ...
      P_NID_COTIZACION: this.inputsQuotation.tipoTransaccion == 1  || this.inputsQuotation.tipoTransaccion ==14 ? paramsTrx[0].P_NID_COTIZACION : paramsTrx.P_NID_COTIZACION,
        ...
        this.cotizacion.COD_MONEDA = this.tramite.COD_MONEDA
...
//apago el flag de pago
    ...
    async onPaymentKushki() {
        ...
        dataCIP.correo = this.cotizacion.contratante.email;
        ...
        dataCIP.Moneda = this.tramite.COD_MONEDA;
...
async InsertPayNCTEMP() { //AVS PRY NC
 ...
 async getCommission(request) : Promise<any>{
  ...
  data.NTYPE_TRANSAC = this.QuotationData.TypeTransac =='Declaración'?  11 : this.nTransac;
