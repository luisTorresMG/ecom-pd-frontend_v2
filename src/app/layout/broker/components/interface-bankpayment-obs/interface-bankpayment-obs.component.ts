...
export class InterfaceBankpaymentObsComponent implements OnInit {
    ...
    inicializarFiltros = () => {
        ...
        this.filtroInterfaz.mvt_npagbnk = 1; // INI MMQ 04-09-2024 - RENTAS DEVOLUCION - FIN
        this.detalle = {};
            ...
            this.InterfaceBankPaymentService.ListarAprobaciones(this.filterForm.getRawValue()).subscribe(
                res => {
                    ...
                    if (this.aprobaciones.length == 0) {
                        Swal.fire('Información', 'Listar Aprobaciones:No se encontraron registros en la búsqueda de aprobaciones.', 'warning');
                    }
                },
                err => {
                    Swal.fire('Información', 'Listar Aprobaciones:Ha ocurrido un error al obtener los registros.', 'error');
                }
            )
        }
    }
    ...
    ListarAprobacionesDetalle(item): void {        
        this.InterfaceBankPaymentService.ListarAprobacionesDetalle(item).subscribe(
            res => {
                ...
                if (this.detalles.length == 0) {
                    Swal.fire('Información', 'Detalle de Aprobaciones:No se encontraron registros en la búsqueda de detalle de aprobaciones.', 'warning');
                }
            },
            err => {
                Swal.fire('Información', 'Detalle de Aprobaciones:Ha ocurrido un error al obtener los registros.', 'error');
            }
        )
    }
        ...
        this.InterfaceBankPaymentService.ListarDetalleObservacion(item).subscribe(
            res => {
                ...
                for (var i = 0; i < this.observaciones.length; i++) {
                    ...
                    this.observaciones[i].MIN_DATE = newDateTime; // Fecha de observación (Formateada)
                }
            },
            err => {
                Swal.fire('Información', 'Ha ocurrido un error al obtener los registros.', 'error');
    ...
    seleccionarAprob = (content: any, item) => {
        ...
        if (item.NSTATUS == 1 || item.NSTATUS == 3) {
            ...
            this.detalle.P_NSTATUS = this.modalService.hasOpenModals() ? null : 3
    ...
    newObs = () => {
        ...
        if (this.agregarObservacion.P_SDES_OBSERV == null || this.agregarObservacion.P_SDES_OBSERV == "") {
            ...
            Swal.fire(
                {
                    title: `Se realizará el guardado y envío de la ' + action + ' al área de ' + origin + '. Se procederá a eliminar el asiento de la ${this.interfazNumber == 9? "Devolución": "Orden de Pago"}.`,
            ...
            for (var i = 0; i < this.observaciones.length; i++) {
                if (this.observaciones[i].NIDPAYBNKO == item.NIDPAYBNKO) {
                    if (item.IS_SELECTED == true) {
                        this.observaciones[i].DFEC_RESULT = this.diaActual;
                    } else {
                        this.observaciones[i].DFEC_RESULT = null;
                    }
                }
            }
        }
        ...
        this.InterfaceBankPaymentService.ResolverObservacionList(item).subscribe(
            res => {
                if (res.Result.P_NCODE == 0) {
                    ...
                    this.ListarAprobacionesDetalle(this.detalle);
                    //this.ListarAprobaciones();
                    if(this.submittedDoc==true){
                        this.ListarAprobacionesDoc();
                    }
                    else{
                        this.ListarAprobaciones();
                    }
                    this.bShowBtnResolver = false
                    this.checkedObsSend[0] = 0;
                } else {
                    Swal.fire('Información', res.Result.P_SMESSAGE, 'error');
                }
    ...
    resolver = () => {
        if (this.checkedObsSend[0].length > 0) {
            for (var i = 0; i < this.checkedObsSend[0].length; i++) {
                if (this.checkedObsSend[0][i].DFEC_RESULT == null || this.checkedObsSend[0][i].DFEC_RESULT == "") {
            ...
            this.InterfaceBankPaymentService.GetHorarioInterfaz(this.detalle).subscribe(
                res => {
                    if (res.Result.P_NCODE == 0) 
                        {
                            console.log(res.Result.P_NHORARIO)
                            console.log(res.Result.P_NHOUR_TODAY)
                        // res.Result.P_NHORARIO;
                        // let curHour = this.getCurrentHour();
                        // let dif = this.compareTimes(curHour, intHour);
                        Swal.fire(
                            {
                                title: (res.Result.P_NHORARIO < res.Result.P_NHOUR_TODAY) ? `Está fuera del horario programado de la interfaz de ${this.interfazNumber == 9? "Devoluciónes": "Órdenes de Pago"} ¿Está seguro que desea continuar?` : '¿Está seguro de resolver los registros seleccionados?',
                                text: 'Esta acción es irreversible.',
                                icon: 'warning',
                                showCancelButton: true,
                                confirmButtonText: 'Aceptar',
                            ...
                            (result) => {
                                if (result.value) {                        
                                    ...
                                    for (var i = 0; i < this.checkedObsSend[0].length; i++) {
                                        ...
                                        item.P_DFEC_APROB = new Date(this.checkedObsSend[0][i].DFEC_RESULT.getFullYear(), this.checkedObsSend[0][i].DFEC_RESULT.getMonth(), this.checkedObsSend[0][i].DFEC_RESULT.getDate());
                                        item.P_CUSERCODE = JSON.parse(localStorage.getItem('currentUser')).username;
