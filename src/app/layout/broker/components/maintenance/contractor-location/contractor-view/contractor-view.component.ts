...
export class ContractorViewComponent implements OnInit {
  ...
  ngOnInit() {
    if (
      ...
      this.contractor.Email.toString().trim() == ''
    )
      this.contractor.Email = 'Email no registrado';
    if (
      this.contractor.Address == null ||
      this.contractor.Address.toString().trim() == ''
    )
      this.contractor.Address = 'Dirección no registrada';
    if (
      this.contractor.Phone == null ||
      this.contractor.Phone.toString().trim() == ''
    )
      this.contractor.Phone = 'Teléfono no registrado';
  ...
  openLocationModal(
    ...
    const modalRef = this.modalService.open(ContractorLocationFormComponent, {
      size: 'lg',
    ...
    modalRef.result.then(
      (shouldToUpdateLocationTable) => {
        if (shouldToUpdateLocationTable == true) {
          // debemos actualizar tabla de SEDES?
          this.getContractorLocationList(this.contractor.Id);
  ...
  getContractorLocation(
    contractorLocationId: string,
    openModalInEditMode: boolean
  ) {
    if (openModalInEditMode == true) {
      this.contractorLocationIndexService
        .getContractorLocation(contractorLocationId)
        .subscribe(
          (res) => {
      ...
      this.contractorLocationIndexService
        .getSuggestedLocationType(this.contractor.Id, this.currentUser)
        .subscribe(
          (res) => {
            if (res.P_NCODE == 1) {
              Swal.fire(
              ...
              if (res.P_NCODE == 2)
                this.openLocationModal(_location, false, '2'); //crear sucursal
              else if (res.P_NCODE == 3)
                this.openLocationModal(_location, false, '1'); //crear principal
            }
          },
          (err) => {
  ...
  getContractorLocationList(contractorId: string) {
    ...
    this.contractorLocationIndexService
      .getContractorLocationList(contractorId, this.itemsPerPage, 1)
      .subscribe(
        (res) => {
          this.locationsWereFound = true;
          this.contractorLocationList = res.GENERICLIST;
          this.totalItems = this.contractorLocationList.length;
          this.listToShow = this.contractorLocationList.slice(
  ...
  deleteLocation(locationId: number) {
    this.contractorLocationIndexService
      .deleteLocation(locationId, this.currentUser)
      .subscribe(
        (res) => {
          if (res.P_NCODE == 0) {
  ...
  stringToList(inputString: string): string[] {
    ...
    if (isFirst == true) {
      //cuando solo hay un mensaje sin guiones
