...
@Component({
  selector: 'app-descd-comisiones-report',
  templateUrl: './descd-comisiones-report.component.html',
  styleUrls: ['./descd-comisiones-report.component.css'],
})
export class DescdComisionesReportComponent implements OnInit {
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportComisionesExcel(
        this.foundResults,
        'ReporteComisiones'
      );
    else
      ...
      if (this.bsValueIni != null && this.bsValueFin != null) {
        if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
          ...
          this.UnselectedItemMessage =
            'La fecha de poliza inicio no puede ser mayor a la fecha de operación fin';
        }

        if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
          this.isError = true;
          this.UnselectedItemMessage =
            'La fecha de poliza fin no puede ser menor a la fecha de operación inicio';
        }
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportsComisiones(data).subscribe(
                  (res) => {
