...
@Component({
  ...
  templateUrl: './vdp-tecnic-report.component.html',
  styleUrls: ['./vdp-tecnic-report.component.css'],
})
export class VdpTecnicReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportTecExcel(
        this.foundResults,
        'ReporteTecnicoVDP'
      );
    else
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportsTecVDP(data).subscribe(
                  (res) => {
