...
@Component({
  standalone: false,
  selector: 'app-vdp-Facturation-report',
  templateUrl: './vdp-Facturation-report.component.html',
  styleUrls: ['./vdp-Facturation-report.component.css'],
})
export class VdpFacturationReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportFacturationExcel(
        this.foundResults,
        'ReporteTecnicoVDP'
      );
    else
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportFacturationVDP(
                  data
                ).subscribe(
                  (res) => {
