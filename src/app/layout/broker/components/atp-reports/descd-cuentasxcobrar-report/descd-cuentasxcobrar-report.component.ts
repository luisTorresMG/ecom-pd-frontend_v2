...
@Component({
  standalone: false,
  selector: 'app-descd-cuentasxcobrar-report',
  templateUrl: './descd-cuentasxcobrar-report.component.html',
  styleUrls: ['./descd-cuentasxcobrar-report.component.css'],
})
export class DescdCuentasxcobrarReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportCuentasxcobrarExcel(
        this.foundResults,
      ...
      if (this.bsValueIni != null && this.bsValueFin != null) {
        if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
          ...
          this.UnselectedItemMessage =
            'La fecha de comprobante inicio no puede ser mayor a la fecha de operación fin';
        }

        if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
          this.isError = true;
          this.UnselectedItemMessage =
            'La fecha de comprobante fin no puede ser menor a la fecha de operación inicio';
        }
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportsCuentasxcobrar(
                  data
                ).subscribe(
                  (res) => {
