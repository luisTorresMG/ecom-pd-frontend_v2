...
export class VdpReserveRegistryReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateVdpReserveRegistryReport(
        this.foundResults,
  ...
  ReportRegistryReserve() {
    ...
    if (this.isError == true) {
      ...
      if (this.bsValueIni != null && this.bsValueFin != null) {
        let frdateIni = new Date(this.bsValueIni);
        let frdateFin = new Date(this.bsValueFin);
        let dateValid = new Date(frdateIni.setMonth(frdateIni.getMonth() + 12));
        let isValid = frdateFin <= dateValid;

        if (!isValid) {
          this.isError = true;
          this.UnselectedItemMessage =
            'Debe consultar hasta 1 año como máximo.';
        }

        if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
          this.isError = true;
          this.UnselectedItemMessage =
            'La fecha de pago inicio no puede ser mayor a la fecha de pago fin';
        }

        if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
          this.isError = true;
          this.UnselectedItemMessage =
            'La fecha de pago fin no puede ser menor a la fecha de pago inicio';
        }
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ReportRegistryReserve(data).subscribe(
                  (res) => {
                    ...
                    if (
                      ...
                      this.excelService.generateVdpReserveRegistryReport(
                        this.foundResults,
                        'Reporte_ReservasReaseguros_VDP_'
                      );
                    } else {
                      this.totalItems = 0;
