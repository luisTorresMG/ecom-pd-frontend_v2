...
@Component({
  standalone: false,
  selector: 'app-vdp-control-report',
  templateUrl: './vdp-control-report.component.html',
  styleUrls: ['./vdp-control-report.component.css'],
})
export class VdpControlReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportControlVDPExcel(
        this.foundResults,
        'Reporte_Polizas_VDP_'
      );
    else
      ...
      if (this.bsValueIni != null && this.bsValueFin != null) {
        if (new Date(this.bsValueIni) > new Date(this.bsValueFin)) {
          ...
          this.UnselectedItemMessage =
            'La fecha de emisión inicio no puede ser mayor a la fecha de emisión fin';
        }

        if (new Date(this.bsValueFin) < new Date(this.bsValueIni)) {
          this.isError = true;
          this.UnselectedItemMessage =
            'La fecha de emisión fin no puede ser menor a la fecha de emisión inicio';
        }
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                if (this.bsValueIni != null && this.bsValueFin != null) {
                  //data.dStart_Date = this.bsValueIni.getDate().toString().padStart(2, '0') + "/" + (this.bsValueIni.getMonth() + 1).toString().padStart(2, '0') + "/" + this.bsValueIni.getFullYear();
                  //data.dExpir_Dat = this.bsValueFin.getDate().toString().padStart(2, '0') + "/" + (this.bsValueFin.getMonth() + 1).toString().padStart(2, '0') + "/" + this.bsValueFin.getFullYear();
                  ...
                  console.log(data.dExpir_Dat);
                } else {
                  data.dStart_Date = null;
                  data.dExpir_Dat = null;
                }

                this.AtpReportService.ReportRegistryPolicies(data).subscribe(
                  (res) => {
