...
@Component({
  selector: 'app-vdp-tecnic-report',
  templateUrl: './descd-tecnic-report.component.html',
  styleUrls: ['./descd-tecnic-report.component.css'],
})
export class DescdTecnicReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportTecnicExcel(
        this.foundResults,
        'ReporteTecnico'
      );
    else
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportsTecnic(data).subscribe(
                  (res) => {
