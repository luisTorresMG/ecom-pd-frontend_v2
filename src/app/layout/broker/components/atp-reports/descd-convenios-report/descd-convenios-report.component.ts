...
@Component({
  selector: 'app-descd-convenios-report',
  templateUrl: './descd-convenios-report.component.html',
  styleUrls: ['./descd-convenios-report.component.css'],
})
export class DescdConveniosReportComponent implements OnInit {
  //CheckBox
  ...
  exportToExcel() {
    if (this.foundResults != null && this.foundResults.length > 0)
      this.excelService.generateReportConveniosExcel(
        this.foundResults,
        'ReporteConvenios'
      );
    else
          ...
          swal
            ...
            .then((result) => {
              if (result.value) {
                ...
                this.AtpReportService.ProcessReportsConvenios(data).subscribe(
                  (res) => {
