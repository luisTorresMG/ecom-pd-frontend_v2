...
export class PanelInfoContratanteComponent implements OnInit {
    ...
    @Output() codTipoCuenta: EventEmitter<any> = new EventEmitter();

    @Input() cotizacion: any;

    @Input() nroCotizacion: any; //RQ2024-332 21/11/2024
    @Input() constractContact: any; //RQ2024-332 21/11/2024
    ...
    //INI RQ2024-332 21/11/2024
    countContract: any;
    countVar: number;
    getDataContratante: any = [];
    checkboxDisabled: any;
    //FIN RQ2024-332 21/11/2024

    longitudContacto: number;


    catchComercialName: any = {}; //RQ2024-180
    paramsComercialName: any = {}; //RQ2024-180
    constructor(
    ...
    ngOnInit() {
        ...
        this.cotizacion.contratante.changes = this.contratChange;

        //INI RQ2024-332 21/11/2024
        this.countContract = false;
        this.countVar = 0;
        if (!this.cotizacion.modoVista == this.CONSTANTS.MODO_VISTA.COTIZAR) {
            this.DataContratante();
        }
        this.catchComercialName = {};
    }

    initConfig() {
        // Configuracion del Template
        this.template = CommonMethods.configuracionTemplate(this.codProducto, this.epsItem.NCODE);
    ...
    checkMovimiento() {
        ...
        if (this.cotizacion.modoVista == this.CONSTANTS.MODO_VISTA.COTIZAR ||
            ...
            flagHabilitar = true;
            //        }else{
            if (this.contratante.checkbox1 != undefined) {
                if (!this.contratante.checkbox1.ASIG_CUSTOM_NAME) {
                    this.contratante.customName = null;
                }
            }
        }
    ...
    validateContracting() {
        ...
        if (this.contratante.codTipoBusqueda === 'POR_DOC') {
            if (!this.contratante.tipoDocumento.Id || !(this.contratante.numDocumento || '').trim()) {
    ...
    async DataContratante() {
        this.countVar += 1;
        if (this.countVar < 2) {
            var paramsComercialName = {
                NRO_COTIZACION: this.nroCotizacion
            };
            try {
                this.getDataContratante = await this.clientInformationService.getDataContratante(paramsComercialName).toPromise();
            } catch (error) {
                console.log(error);
            } finally {
                this.constractContact.checkbox1.CONTACT_CONTRACT = this.getDataContratante.P_NFLATCONTACT == 1 ? false : true;
                this.contratante.customName = this.getDataContratante.SCOMERCIAL_NAME;
                if (this.contratante.customName != null) {
                    this.contratante.checkbox1.ASIG_CUSTOM_NAME = true;
                }
            }
        }
    }
    ...
    async buscarContratante() {
        ...
        this.flagJuridico = (this.contratante.tipoDocumento || {}).Id == this.CONSTANTS.TIPO_DOCUMENTO.RUC && (this.contratante.numDocumento || '').toUpperCase().substr(0, 2) == '20' ? true : false;
        let dataQuality = await this.validateContractingData(params);

        if (dataQuality.P_NCODE == 0) {
            this.countContract = true;
            if (dataQuality.EListClient.length > 0) {
                if (dataQuality.EListClient.length == 1) {
                    if (dataQuality.EListClient[0].P_SCLIENT == null) {
                        ...
                        if (params.P_NIDDOC_TYPE === this.CONSTANTS.TIPO_DOCUMENTO.RUC) {
                            if (dataQuality.EListClient[0].P_SLEGALNAME === null) {
                                swal.fire('Información', 'No es posible validar el número de documento ingresado. Por favor, verificar que la información sea correcta', 'error');
                                this.isLoadingChange.emit(false);
                            } else {
                                // if ((dataQuality.EListClient[0].P_SCONDDOMICILIO == 'HABIDO' && dataQuality.EListClient[0].P_SESTADOCONTR == 'ACTIVO' &&
                                //    this.flagJuridico) || !this.flagJuridico) {
                                if (this.storageService.esBroker) {
                                    this.agregaContratante(params.P_NIDDOC_TYPE, params.P_SIDDOC, dataQuality.P_NCODE, dataQuality);
                                } else {
                                    this.agregaContratante(params.P_NIDDOC_TYPE, params.P_SIDDOC, dataQuality.P_NCODE, null);
                                }
                                // } else {
                                //     swal.fire('Información', 'El RUC ' + this.contratante.numDocumento + ' no está activo o habilitado en la SUNAT. Por favor, verifica que la información sea correcta.', 'error');
                                //     this.isLoadingChange.emit(false);
                                // }
                            }
                        } else {
                            if (dataQuality.EListClient[0].P_SFIRSTNAME === null && dataQuality.EListClient[0].P_SLASTNAME === null && dataQuality.EListClient[0].P_SLASTNAME2 === null) {
                                swal.fire('Información', 'No es posible validar el número de documento ingresado. Por favor, verificar que la información sea correcta', 'error');
                                this.isLoadingChange.emit(false);
                            } else {
                                if (this.storageService.esBroker) {
                        ...
                        if (dataQuality.EListClient[0].P_SIDDOC != null) {
                            let flag = this.CONSTANTS.MODO_VISTA.POLIZARENOVAR !== this.cotizacion.modoVista &&
                            ...
                            // if ((dataQuality.EListClient[0].P_SCONDDOMICILIO == 'HABIDO' && dataQuality.EListClient[0].P_SESTADOCONTR == 'ACTIVO' &&
                            //    this.flagJuridico) || !this.flagJuridico || this.modoVista != this.CONSTANTS.MODO_VISTA.COTIZAR) {

                            let params: any = {
                                branchCode: this.CONSTANTS.RAMO,
                                productCode: this.CONSTANTS.COD_PRODUCTO,
                                clientCode: dataQuality.EListClient[0].P_SCLIENT,
                                ...
                                nidCotizacion: flag ? this.cotizacion.numeroCotizacion : 0
                            };
                            ...
                            this.cargarDatosCliente(dataQuality);
                                ...
                                if (this.modoVista == this.CONSTANTS.MODO_VISTA.EXCLUIR) {
                                    ...
                                    await this.cobranzasService.validateDebt(params).toPromise().then(
                                        res => {
                                            ...
                                            this.contratante.deudacip = res.deudacip;
                                            ...
                                            if (this.modoVista !== this.CONSTANTS.MODO_VISTA.VISUALIZAR) {
                                                if (res.lockMark !== this.CONSTANTS.USUARIO_OK) {
                                                    ...
                                                    if (this.contratante.deudacip != 99) {
                                                        this.generarReporteCliente(params, res);
                                                    }
                                                } else {
                                                    if (this.CONSTANTS.MODO_VISTA.POLIZARENOVAR == this.modoVista || this.CONSTANTS.MODO_VISTA.ENDOSO == this.modoVista) {
                    ...
                    modalRef.componentInstance.EListClient = dataQuality.EListClient;
                    ...
                    modalRef.result.then(async (dataContratante) => {
                        if (dataContratante !== undefined) {
                            ...
                            const dataBlock = {
                                branchCode: this.CONSTANTS.RAMO,
                                productCode: this.CONSTANTS.COD_PRODUCTO,
                                documentType: dataContratante.P_NIDDOC_TYPE,
                                documentNumber: dataContratante.P_SIDDOC.toUpperCase()
                            };

                            const response: any = await this.validarBloqueado(dataBlock);
                            if (response.lockMark === this.CONSTANTS.USUARIO_BLOQUEADO) {
                                swal.fire('Información', response.observation, 'error');
                                return;
                            }
                            ...
                            data.P_NBRANCH = this.CONSTANTS.RAMO;
                ...
                this.isLoadingChange.emit(false);
                this.codTipoCuenta.emit(this.contratante.codTipoCuenta)
            }
        } else if (dataQuality.P_NCODE === this.CONSTANTS.TIPO_RESPUESTA.SUNAT_ERROR) {
        ...
        //                     // } else {
        //                     if (params.P_NIDDOC_TYPE === this.CONSTANTS.TIPO_DOCUMENTO.RUC) {
        //                         if (resp.EListClient[0].P_SLEGALNAME === null) {
        //                             swal.fire('Información', 'No es posible validar el número de documento ingresado. Por favor, verificar que la información sea correcta', 'error');
        //                             this.isLoadingChange.emit(false);
        //                         } else {
        //                             if ((resp.EListClient[0].P_SCONDDOMICILIO == 'HABIDO' && resp.EListClient[0].P_SESTADOCONTR == 'ACTIVO' &&
        //                                 this.flagJuridico) || !this.flagJuridico) {
        //                                 if (this.storageService.esBroker) {
        //                                     this.agregaContratante(params.P_NIDDOC_TYPE, params.P_SIDDOC, resp.P_NCODE, resp);
        //                                 } else {
        ...
        //                         }
        //                     } else {
        //                         if (resp.EListClient[0].P_SFIRSTNAME === null &&
        //                             resp.EListClient[0].P_SLASTNAME === null &&
        //                             resp.EListClient[0].P_SLASTNAME2 === null) {
        //                             swal.fire('Información', 'No es posible validar el número de documento ingresado. Por favor, verificar que la información sea correcta', 'error');
        //                         } else {
        //                             if (this.storageService.esBroker) {
        //                                 this.agregaContratante(params.P_NIDDOC_TYPE, params.P_SIDDOC, resp.P_NCODE, resp);
    ...
    generateRequestCDatos() {
        ...
        data.P_TIPOPER = 'CON';
        data.P_NUSERCODE = JSON.parse(localStorage.getItem('currentUser'))['id'];
        data.P_NIDDOC_TYPE = (this.contratante.tipoDocumento || {}).Id;
        data.P_SIDDOC = (this.contratante.numDocumento || '').toUpperCase();
        data.P_SFIRSTNAME = (this.contratante.nombres || '').toUpperCase();
        data.P_SLASTNAME = (this.contratante.apellidoPaterno || '').toUpperCase();
        data.P_SLASTNAME2 = (this.contratante.apellidoMaterno || '').toUpperCase();
        data.P_SLEGALNAME = (this.contratante.razonSocial || '').toUpperCase();
        data.P_NBRANCH = this.CONSTANTS.RAMO;
    ...
    async validateContractingData(data: any): Promise<any> {
        console.log(this.contratante.codTipoBusqueda);
    ...
    async agregaContratante(documentType, documentNumber, ncode, response) {
        ...
        if (this.storageService.esBroker) {
            ...
            contracting.P_NBRANCH = this.CONSTANTS.RAMO;
            contracting.EListAddresClient = response.EListClient[0].EListAddresClient;
            contracting.EListAddresClient = contracting.EListAddresClient.length > 0 ? await this.getAddress(contracting) : null;
            contracting.EListPhoneClient = null;
            contracting.EListEmailClient = null;
            contracting.EListContactClient = null;
            contracting.EListCIIUClient = null;
            ...
            this.clientInformationService.insertContractingData(contracting).toPromise().then(
                res => {
                    if (res.P_NCODE === '0') {
                        const params = {
                            P_TipOper: 'CON',
                        ...
                        this.clientInformationService.validateContractingData(params).toPromise().then(
                            res => {
    ...
    async cargarDatosCliente(res) {
        ...
        // contractingdata.P_NBRANCH = this.CONSTANTS.RAMO;
        ...
        this.contratante.flagContact = (this.contratante.tipoDocumento || {}).Id == this.CONSTANTS.TIPO_DOCUMENTO.RUC && (this.contratante.numDocumento || '').toUpperCase().substr(0, 2) == '20' ? true : contractingdata.EListContactClient.length > 0 ? false : true;
        // console.log("this.contratante.flagContact ", this.contratante.flagContact);
        ...
        if (this.template.ins_historialCreditoQuotation) {
            if (!this.cotizacion.esEstudiantil) {
                const data: any = {};
                ...
                await this.clientInformationService.invokeServiceExperia(data).toPromise().then(
                    res => {
                        ...
                        this.contratante.creditHistory.nflag_bk = res.nflag_bk;
                ...
                this.contratante.creditHistory.nflag_bk = 0;
                this.contratante.creditHistory.sdescript = "";
                // (this.cotizacion.cotizador.checkbox || {}).PAGO_ADEL = (this.contratante.creditHistory.nflag == 0);
            }
        } else {
            this.contratante.creditHistory = {};
        ...
        if (!this.detail) { //JDD - AP VG
            (this.cotizacion.cotizador.checkbox || {}).PAGO_ADEL = (this.contratante.creditHistory.nflag == 0);
        }

        this.cotizacion.contratante.changes = this.contratante.id;
    }
    ...
    changeAsignar() {
        this.contratante.seccionDocumento = this.contratante.checkbox1.ASIG_CUSTOM_NAME ? this.contratante.seccionDocumento : '2'
        this.contratanteChange.emit(this.contratante);
    }
