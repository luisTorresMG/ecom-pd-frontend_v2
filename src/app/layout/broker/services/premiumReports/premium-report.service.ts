...
export class PremiumReportService {
    ...
    public GetBranchesPremiumReport(data: any): Observable<any> {
        const _params = { P_TIPO: data }
        return this.http.get(this.Url + '/ReportKuntur/branchPremiumReport', {
            params: _params
        });
    }
    //Obtener Lista de Productos
    public GetProductPremiumReport(data: any, IdBranch: any): Observable<any> {
        const _params = { P_TIPO: data, P_NBRANCH: IdBranch }
        return this.http.get(this.Url + '/ReportKuntur/productPremiumReport', {
            params: _params
        });
    }
    //Obtener Estados
    public GetStatusPremiumReport(): Observable<any> {
        return this.http.get(this.Url + '/ReportKuntur/statusPremiumReport', {
    ...
    public GetListReports(data: any): Observable<any> {
        ...
        return this.http.post(this.Url + '/ReportKuntur/listPremiumReport', body, {
            headers: this.headers
        });
    }
    //Descarga  de reporte o reportes
    public GetFilePremiumReport(id: any): Observable<any> {
        const Json = { "id": id };
        const data = JSON.stringify(Json);
        return this.http.post(this.Url + '/ReportKuntur/getFilePremiumReport', data,
            {
                headers: this.headers
            });
    }
    ...
    private saveAsExcelFile(buffer: any, fileName: string): void {
        ...
        FileSaver.saveAs(
            data,
            fileName + "_generados_cierre_" + ("0" + new Date().getDate()).slice(-2) + ("0" + (new Date().getMonth() + 1)).slice(-2) + new Date().getFullYear() +
    ...
    public ConvertListToPrintPDF(json: any[]): void {
        ...
        const nameReport = 'Reportes_generados_cierre_' + ("0" + new Date().getDate()).slice(-2) + ("0" + (new Date().getMonth() + 1)).slice(-2) + new Date().getFullYear() +
    ...
    public ProcessReports(data: any): Observable<any> {
        const Json = {
            ...
            "fecInicio": data.fecInicio, "fecFin": data.fecFin , "idProcesoOr" : data.idProcesoOr
        };
        return this.http.post(this.Url + '/ReportKuntur/processPremiumReport', Json,
            {
                headers: this.headers
            });
    }
