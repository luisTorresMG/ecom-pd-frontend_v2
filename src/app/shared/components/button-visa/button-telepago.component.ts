...
@Component({
    selector: 'app-button-telepago',
    templateUrl: './button-visa.component.html',
    styleUrls: ['./button-visa.component.css']
})

export class ButtonTelepagoComponent implements OnInit, AfterViewInit, OnDestroy {
    @Input()
    action: string;
    isIEOrEdge: any;
    method = 'POST';
    ...
    sessionToken: string;
    subscribe;
    merchantId = environment.codigocomercioTP;
    ...
    private InjectFormulario() {
        ...
        const div = document.body.getElementsByClassName('btn-telepago');
        if (div[0]) {
        ...
        this.form.setAttribute('id', 'frmtelepago');
        ...
        script.setAttribute('data-channel', 'callcenter');
        script.setAttribute('data-expirationminutes', '20');
        script.setAttribute('data-timeouturl', this.timeouturl);
